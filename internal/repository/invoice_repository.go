...
func (r *InvoiceRepository) Save(invoice *domain.Invoice) error {
	...
	(
		...
		$6,
		$7
	)`, invoice.ID, invoice.AccountID, invoice.Amount, invoice.Status, invoice.Description, invoice.CreatedAt, invoice.UpdatedAt)
...
func (r *InvoiceRepository) UpdateStatus(invoice *domain.Invoice) error {
	...
	SET status = $1,
		updated_at = $2
	WHERE id = $3`, invoice.Status, invoice.UpdatedAt, invoice.ID)
